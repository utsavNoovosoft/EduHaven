...
function UserList({ users, selectedUser, onSelectUser }) {
  ...
  return (
    <div className="h-full flex flex-col" style={{ backgroundColor: 'color-mix(in srgb, var(--bg-sec), black 10%)' }}>
      {/* Header - Responsive */}
      <div className="p-2 sm:p-3 lg:p-4 border-b border-gray-200/10">
        <h2 className="text-lg sm:text-xl font-semibold txt mb-2 sm:mb-4">Messages</h2>
        ...
        <div className="relative">
          ...
          <input
            ...
            style={{ backgroundColor: 'color-mix(in srgb, var(--bg-ter), black 8%)' }}
          />
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        {filteredUsers.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full txt-disabled">
            ...
            <p className="text-xs sm:text-sm text-center px-2">No conversations</p>
          </div>
        ) : (
          filteredUsers.map((user) => (
            <div
              ...
              className={`p-2 sm:p-3 lg:p-4 cursor-pointer transition-colors border-b border-gray-200/5 hover:opacity-80 ${
                selectedUser?.id === user.id ? 'border-l-4 border-l-[var(--btn)]' : ''
              }`}
              style={{ 
              ...
              <div className="flex items-center gap-2 sm:gap-3">
                ...
                <div className="relative flex-shrink-0">
                  ...
                  {user.isOnline != undefined && (
                    <div className={`absolute -bottom-0.5 -right-0.5 sm:-bottom-1 sm:-right-1 w-2.5 h-2.5 sm:w-3 sm:h-3 lg:w-4 lg:h-4 rounded-full border-2 border-sec ${
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-0.5 sm:mb-1">
                    <h3 className="font-medium txt truncate text-xs sm:text-sm lg:text-base">{user.name}</h3>
                    <span className="text-xs txt-disabled hidden sm:block">{user.timestamp}</span>
                  </div>
