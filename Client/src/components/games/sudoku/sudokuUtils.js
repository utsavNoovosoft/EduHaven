...
export const boardsEqual = (a, b) => a.every((row, i) => row.every((v, j) => v === b[i][j]));
...
function generateSolved() {
  const rows = shuffle([0, 1, 2]).flatMap((r) => shuffle([0, 1, 2]).map((c) => 3 * r + c));
  const cols = shuffle([0, 1, 2]).flatMap((r) => shuffle([0, 1, 2]).map((c) => 3 * r + c));
  const nums = shuffle([1, 2, 3, 4, 5, 6, 7, 8, 9]);
  const board = Array.from({ length: 9 }, (_, r) =>
    Array.from({ length: 9 }, (_, c) => nums[(basePattern(rows[r], cols[c]) + 0) % 9])
  );
  return board;
}
